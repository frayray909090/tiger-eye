...
local customdir = (shared.TigerPrivate and "Tiger_eye/" or "Tiger/")
	...
	if shared.TigerDeveloper then
		...
		task.delay(15, function()
			if res == nil and (not shown) then 
				...
				prompt:setErrorTitle("Tiger_eye")
		...
		res = game:HttpGet("https://raw.githubusercontent.com/frayray909090/tiger-eye-prison-life/main/"..scripturl, true)
...
if not (getasset and requestfunc and queueteleport) then
	print("Tiger not supported with your exploit.")
	return
end

if shared.TigerExecuted then
	error("Tiger Already Injected")
	return
else
	shared.TigerExecuted = true
end
local redownload = false
if isfolder(customdir:gsub("/", "")) == false then
	makefolder(customdir:gsub("/", ""))
end
if isfolder("Tiger") == false then
	makefolder("Tiger")
end
if not betterisfile("Tiger/assetsversion.dat") then
	writefile("Tiger/assetsversion.dat", "1")
end
...
if not betterisfile("Tiger/language.dat") then
	writefile("Tiger/language.dat", "en-us")
end
if isfolder("Tiger/assets") == false then
	makefolder("Tiger/assets")
end
task.spawn(function()
	local assetver = checkassetversion()
	if assetver and assetver > readfile("Tiger/assetsversion.dat") then
		redownload = true
		if isfolder("Tiger/assets") and shared.TigerDeveloper == nil then
			if delfolder then
				delfolder("Tiger/assets")
				makefolder("Tiger/assets")
			end
		end
		writefile("Tiger/assetsversion.dat", assetver)
	end
end)
...
checkpublicrepo = function(id)
	local suc, req = pcall(function() return requestfunc({
		Url = "https://raw.githubusercontent.com/frayray909090/tiger-eye-prison-life/main/CustomModules/"..id..".lua",
		Method = "GET"
	}) end)
	if not suc then
...
task.spawn(function()
	...
	image.Image = getcustomassetfunc("Tiger/assets/CombatIcon.png")
	...
	task.delay(15, function()
		if image.ContentImageSize == Vector2.new(0, 0) and (not shown) and (not redownload) and (not betterisfile("Tiger/assets/check3.txt")) then 
			shown = true
			local ErrorPrompt = getrenv().require(game:GetService("CoreGui").RobloxGui.Modules.ErrorPrompt)
			local prompt = ErrorPrompt.new("Default")
			prompt._hideErrorCode = true
			local gui = Instance.new("ScreenGui", game:GetService("CoreGui"))
			prompt:setParent(gui)
			prompt:setErrorTitle("Tiger")
			prompt:updateButtons({{
				Text = "OK",
				Callback = function() 
					prompt:_close() 
					writefile("Tiger/assets/check3.txt", "")
				end,
				Primary = true
			}}, 'Default')
			prompt:_open("Tiger has detected that you have a skill issue and cannot load assets, Consider getting a better executor.")
		end
	end)
end)
...
local Combat = GuiLibrary.CreateWindow({
	["Name"] = "Combat", 
	["Icon"] = "Tiger/assets/CombatIcon.png", 
	["IconSize"] = 15
})
local Blatant = GuiLibrary.CreateWindow({
	["Name"] = "Blatant", 
	["Icon"] = "Tiger/assets/BlatantIcon.png", 
	["IconSize"] = 16
})
local Render = GuiLibrary.CreateWindow({
	["Name"] = "Render", 
	["Icon"] = "Tiger/assets/RenderIcon.png", 
	["IconSize"] = 17
})
local Utility = GuiLibrary.CreateWindow({
	["Name"] = "Utility", 
	["Icon"] = "Tiger/assets/UtilityIcon.png", 
	["IconSize"] = 17
})
...
local Profiles = GuiLibrary.CreateWindow2({
	["Name"] = "Profiles", 
	["Icon"] = "Tiger/assets/ProfilesIcon.png", 
	["IconSize"] = 19
})
GUI.CreateDivider()
GUI.CreateButton({
...
ProfilesTextList = Profiles.CreateTextList({
	...
	["RemoveFunction"] = function(num, obj) 
		if obj ~= "default" and obj ~= GuiLibrary["CurrentProfile"] then 
			pcall(function() delfile(customdir.."Profiles/"..obj..(shared.CustomSaveTiger or game.PlaceId)..".Tigerprofile.txt") end)
...
OnlineProfilesButtonImage.Image = getcustomassetfunc("Tiger/assets/OnlineProfilesButton.png")
...
OnlineProfilesExitButton.Image = getcustomassetfunc("Tiger/assets/ExitIcon1.png")
...
OnlineProfilesFrameShadow.Image = getcustomassetfunc("Tiger/assets/WindowBlur.png")
...
OnlineProfilesFrameIcon.Image = getcustomassetfunc("Tiger/assets/ProfilesIcon.png")
...
OnlineProfilesButton.MouseButton1Click:Connect(function()
	...
	if profilesloaded == false then
		...
		local saveplaceid = tostring(shared.CustomSaveTiger or game.PlaceId)
		for i,v in pairs(grabdata("https://raw.githubusercontent.com/7GrandDadPGN/TigerProfiles/main/Profiles/"..saveplaceid.."/profilelist.txt")) do 
			onlineprofiles[i] = v
		end
		for i2,v2 in pairs(onlineprofiles) do
			local profileurl = "https://raw.githubusercontent.com/7GrandDadPGN/TigerProfiles/main/Profiles/"..saveplaceid.."/"..v2.OnlineProfileName
			...
			profiledownload.MouseButton1Click:Connect(function()
				writefile(customdir.."Profiles/"..v2["ProfileName"]..saveplaceid..".Tigerprofile.txt", game:HttpGet(profileurl, true))
...
local TextGui = GuiLibrary.CreateCustomWindow({
	["Name"] = "Text GUI", 
	["Icon"] = "Tiger/assets/TextGUIIcon1.png", 
	["IconSize"] = 21
})
local TextGuiCircleObject = {["CircleList"] = {}}
GUI.CreateCustomToggle({
	["Name"] = "Text GUI", 
	["Icon"] = "Tiger/assets/TextGUIIcon3.png",
...
onething.Image = getcustomassetfunc(translatedlogo and "Tiger/translations/"..GuiLibrary["Language"].."/TigerLogo3.png" or "Tiger/assets/TigerLogo3.png")
...
onething2.Image = getcustomassetfunc("Tiger/assets/TigerLogo4.png")
...
textguimode = TextGui.CreateDropdown({
	...
	["Function"] = function(val)
		...
		if val == "Drawing" then
			...
			onethingdrawing.Data = readfile(translatedlogo and "Tiger/translations/"..GuiLibrary["Language"].."/TigerLogo3.png" or "Tiger/assets/TigerLogo3.png")
			...
			onething2drawing.Data = readfile("Tiger/assets/TigerLogo4.png")
			...
			onething3drawing.Data = readfile(translatedlogo and "Tiger/translations/"..GuiLibrary["Language"].."/TigerLogo3.png" or "Tiger/assets/TigerLogo3.png")
			...
			onething4drawing.Data = readfile("Tiger/assets/TigerLogo4.png")
...
TextGui.CreateToggle({
	...
	["HoverText"] = "Renders a Tiger watermark"
})
...
local TargetInfo = GuiLibrary.CreateCustomWindow({
	["Name"] = "Target Info",
	["Icon"] = "Tiger/assets/TargetInfoIcon1.png",
	["IconSize"] = 16
})
...
healthbarbkgshadow.Image = getcustomassetfunc("Tiger/assets/WindowBlur.png")
...
GUI.CreateCustomToggle({
	["Name"] = "Target Info", 
	["Icon"] = "Tiger/assets/TargetInfoIcon2.png", 
...
local teleportfunc = game:GetService("Players").LocalPlayer.OnTeleport:Connect(function(State)
    if (not teleported) and (not shared.TigerIndependent) then
		teleported = true
		local teleportstr = 'shared.TigerSwitchServers = true if shared.TigerDeveloper then loadstring(readfile("Tiger/NewMainScript.lua"))() else loadstring(game:HttpGet("https://raw.githubusercontent.com/frayray909090/tiger-eye-prison-life/main/NewMainScript.lua", true))() end'
		if shared.TigerDeveloper then
			teleportstr = 'shared.TigerDeveloper = true '..teleportstr
		end
		if shared.TigerPrivate then
			teleportstr = 'shared.TigerPrivate = true '..teleportstr
		end
		if shared.TigerCustomProfile then 
			teleportstr = "shared.TigerCustomProfile = '"..shared.TigerCustomProfile.."'"..teleportstr
		end
		GuiLibrary["SaveSettings"]()
...
GuiLibrary["SelfDestruct"] = function()
	...
	shared.TigerExecuted = nil
	shared.TigerPrivate = nil
	shared.TigerFullyLoaded = nil
	shared.TigerSwitchServers = nil
	shared.GuiLibrary = nil
	shared.TigerIndependent = nil
	shared.TigerManualLoad = nil
	shared.CustomSaveTiger = nil
...
GeneralSettings.CreateButton2({
	...
	["Function"] = function()
		...
		local id = (shared.CustomSaveTiger or game.PlaceId)
		GuiLibrary["SelfDestruct"]()
		delfile(customdir.."Profiles/"..(GuiLibrary["CurrentProfile"] == "default" and "" or GuiLibrary["CurrentProfile"])..id..".Tigerprofile.txt")
		shared.TigerSwitchServers = true
		shared.TigerOpenGui = true
...
if shared.TigerIndependent then
	spawn(function()
		repeat task.wait() until shared.TigerManualLoad
		GuiLibrary["LoadSettings"](shared.TigerCustomProfile)
		...
		UpdateHud()
		if not shared.TigerSwitchServers then
			if blatantmode["Enabled"] then
				pcall(function()
					local frame = GuiLibrary["CreateNotification"]("Blatant Enabled", "Tiger is now in Blatant Mode.", 5.5, "assets/WarningNotification.png")
		...
		if shared.TigerOpenGui then
			GuiLibrary["MainGui"].ScaledGui.ClickGui.Visible = true
			game:GetService("RunService"):SetRobloxGuiFocused(GuiLibrary["MainBlur"].Size ~= 0) 
			shared.TigerOpenGui = nil
		end

		coroutine.resume(selfdestructsave)
	end)
	shared.TigerFullyLoaded = true
	return GuiLibrary
else
	loadstring(GetURL("AnyGame.lua"))()
	if betterisfile("Tiger/CustomModules/"..game.PlaceId..".lua") then
		loadstring(readfile("Tiger/CustomModules/"..game.PlaceId..".lua"))()
	else
	...
	if shared.TigerPrivate then
		if pcall(function() readfile("Tigerprivate/CustomModules/"..game.PlaceId..".lua") end) then
			loadstring(readfile("Tigerprivate/CustomModules/"..game.PlaceId..".lua"))()
		end	
	end
	GuiLibrary["LoadSettings"](shared.TigerCustomProfile)
	local profiles = {}
