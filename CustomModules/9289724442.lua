...
do
	...
	entity.isPlayerTargetable = function(plr)
		return lplr ~= plr and shared.Tigerteamcheck(plr) and friendCheck(plr) == nil
	end
					...
					for i,plr in pairs(players:GetChildren()) do
						...
						if ESPDrawing["Enabled"] then 
							if ESPMethod["Value"] == "2D" then
								...
								if isAlive(plr) and plr ~= lplr and (ESPTeammates["Enabled"] or shared.Tigerteamcheck(plr)) then
				...
				BindToRenderStep("NameTags", 500, function()
					for i,plr in pairs(players:GetChildren()) do
						...
						if NameTagsDrawing["Enabled"] then
							...
							if isAlive(plr) and plr ~= lplr and (NameTagsTeammates["Enabled"] or shared.Tigerteamcheck(plr)) then
				...
				BindToRenderStep("Tracers", 500, function()
					for i,plr in pairs(players:GetChildren()) do
							...
							if TracersDrawing["Enabled"] then
								...
								if isAlive(plr) and plr ~= lplr and (TracersTeammates["Enabled"] or shared.Tigerteamcheck(plr)) then
...
runcode(function()
	...
	searchNewHighlight = searchModule.CreateToggle({
		...
		["Function"] = function(callback)
			if callback then
				local suc = pcall(function() return readfile("Tiger/Profiles/HighlightCheck.Tigersetting.txt") end)
				if not suc then
					if searchNewHighlight["Enabled"] then
					...
					frameIcon.Image = getcustomassetfunc("Tiger/assets/ProfilesIcon.png")
					...
					frameShadow.Image = getcustomassetfunc("Tiger/assets/WindowBlur.png")
					...
					frameExitButton.Image = getcustomassetfunc("Tiger/assets/ExitIcon1.png")
					...
					framebutton1.MouseButton1Click:connect(function()
						spawn(function()
							setclipboard("https://github.com/7GrandDadPGN/TigerV4ForRoblox/wiki/Turning-on-Highlights")
					...
					framebutton2.MouseButton1Click:connect(function()
						frame:Remove()
						writefile("Tiger/Profiles/HighlightCheck.Tigersetting.txt", "")
