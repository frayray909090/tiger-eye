...
local targetinfo = shared.TigerTargetInfo
...
local function GetURL(scripturl)
	if shared.TigerDeveloper then
		...
		return game:HttpGet("https://raw.githubusercontent.com/frayray909090/tiger-eye-prison-life/main/"..scripturl, true)
	end
end
local entity = shared.Tigerentity
local WhitelistFunctions = shared.Tigerwhitelist
...
local function CreateAutoHotbarGUI(children2, argstable)
	...
	ItemListExitButton.Image = getcustomassetfunc("Tiger/assets/ExitIcon1.png")
	...
	ItemListFrameShadow.Image = getcustomassetfunc("Tiger/assets/WindowBlur.png")
...
runcode(function()
    getfunctions = function()
		...
		if not shared.Tigerbypassed then
			...
			shared.Tigerbypassed = true
		end

		task.spawn(function()
			local chatsuc, chatres = pcall(function() return game:GetService("HttpService"):JSONDecode(readfile("Tiger/Profiles/bedwarssettings.json")) end)
			if chatsuc then
				if chatres.crashed and (not chatres.said) then
					pcall(function()
						createwarning("Tiger", "either ur poor or its a exploit moment", 10)
						createwarning("Tiger", "getconnections crashed, chat hook not loaded.", 10)
					end)
					local jsondata = game:GetService("HttpService"):JSONEncode({
						crashed = true,
						said = true,
					})
					writefile("Tiger/Profiles/bedwarssettings.json", jsondata)
				end
											...
											MessageData.ExtraData = {
												...
												Tags = {
													table.unpack(MessageData.ExtraData.Tags),
													{
														...
														TagText = "Tiger PRIVATE"
													}
												}
											}
										end
										if plrtype == "Tiger OWNER" then
											MessageData.ExtraData = {
...
connectionstodisconnect[#connectionstodisconnect + 1] = lplr.OnTeleport:Connect(function(State)
    if State == Enum.TeleportState.Started then
		...
		if clientstorestate.Party and clientstorestate.Party.members and #clientstorestate.Party.members > 0 then
        	queuedstring = queuedstring..'shared.Tigerteammembers = '..#clientstorestate.Party.members..'\n'
		end
		if tpstring then
			queuedstring = queuedstring..'shared.Tigeroverlay = "'..tpstring..'"\n'
		end
...
local function getNametagString(plr)
	...
	if WhitelistFunctions:CheckPlayerType(plr) == "Tiger PRIVATE" then
		nametag = '<font color="rgb(127, 0, 255)">[Tiger PRIVATE] '..(plr.Name)..'</font>'
	end
	if WhitelistFunctions:CheckPlayerType(plr) == "Tiger OWNER" then
		nametag = '<font color="rgb(255, 80, 80)">[Tiger OWNER] '..(plr.DisplayName or plr.Name)..'</font>'
	end
		...
		if (WhitelistFunctions:CheckPlayerType(plr) ~= "DEFAULT" or WhitelistFunctions.WhitelistTable.chattags[WhitelistFunctions:Hash(plr.Name..plr.UserId)]) then
			...
			if playerlist then
				pcall(function()
					...
					if targetedplr then 
						targetedplr.ChildrenFrame.NameFrame.BGFrame.OverlayFrame.PlayerIcon.Image = getcustomassetfunc("Tiger/assets/TigerIcon.png")
					end
				end)
			end
			if lplr ~= plr and WhitelistFunctions:CheckPlayerType(lplr) == "DEFAULT" then
				task.spawn(function()
					...
					task.wait(4)
					repstorage.DefaultChatSystemChatEvents.SayMessageRequest:FireServer("/w "..plr.Name.." "..clients.ChatStrings2.Tiger, "All")
					task.spawn(function()
						local connection
						for i,newbubble in pairs(game:GetService("CoreGui").BubbleChat:GetDescendants()) do
							if newbubble:IsA("TextLabel") and newbubble.Text:find(clients.ChatStrings2.Tiger) then
				...
				if char then
					task.spawn(function()
						pcall(function() 
							...
							Cape(char, getcustomassetfunc("Tiger/assets/TigerCape.png"))
						end)
					end)
				end
			end
								...
								if pos then
									RunLoops:BindToHeartbeat("AntiVoid", 1, function(dt)
										if entity.isAlive and entity.character.Humanoid.Health > 0 and networkownerfunc(entity.character.HumanoidRootPart) and (entity.character.HumanoidRootPart.Position - pos).Magnitude > 1 and AntiVoid["Enabled"] then 
											...
											local realnewpos = CFrame.new(newpos == newpos and entity.character.HumanoidRootPart.CFrame.p + (newpos * (3 * dt)) or Vector3.zero)
                                    ...
                                    entity.character.HumanoidRootPart.CFrame += Vector3.new(0, 100000, 0)
                                    antivoiding = false
								end
							end
						end
					...
					RunLoops:BindToHeartbeat("LongJump", 1, function(dt)
						if entity.isAlive then 
							...
							if startpos == nil then 
								startpos = entity.character.HumanoidRootPart.Position
							end
							...
							if newval then 
								...
								damagetimertick = damagetimertick - dt
								if damagetimertick < 1.2 then 
									damagetimer = 0.001
								end
								...
								entity.character.HumanoidRootPart.Velocity = newvelo
				...
				RunLoops:BindToHeartbeat("Spider", 1, function()
					if entity.isAlive then
						if spidermode["Value"] == "Normal" then
							local vec = entity.character.Humanoid.MoveDirection * 2
							...
							local newray2, newray2pos = getblock(entity.character.HumanoidRootPart.Position + ((entity.character.HumanoidRootPart.CFrame.lookVector * 1.5) - Vector3.new(0, entity.character.Humanoid.HipHeight, 0)))
							if newray2 and (not newray2.CanCollide) then newray2 = nil end 
							if newray2 then 
								newray2pos = newray2pos * 3
								local newpos = roundpos(entity.character.HumanoidRootPart.Position, Vector3.new(newray2pos.X, math.min(entity.character.HumanoidRootPart.Position.Y, newray2pos.Y), newray2pos.Z), Vector3.new(1.1, 1.1, 1.1))
...
runcode(function()
	...
	local killauranoTiger = {["Enabled"] = false}
	...
	local function newAttackEntity(plr, firstplayercodedone, attackedplayers)
		...
		if killauranoTiger["Enabled"] and clients.ClientUsers[plr.Player.Name] then
			return nil
		end
  ...
  --[[  killauratargets = Killaura.CreateSlider({
	...
	if WhitelistFunctions:CheckPlayerType(lplr) ~= "DEFAULT" then
		killauranoTiger = Killaura.CreateToggle({
			["Name"] = "No Tiger",
			["Function"] = function() end,
			["HoverText"] = "no hit Tiger user"
		})
	end
end)
...
runcode(function()
	...
	local function shoot(item, ammotypething)
		...
		if queueType == "winter_event" and entity.isAlive then 
			local boss = workspace:FindFirstChild("WinterEventBoss")
			if boss and (boss.PrimaryPart.Position - entity.character.HumanoidRootPart.Position).Magnitude < 300 then 
		...
		if plr then 
			...
			local ammo = tab.projectileType(ammotypething)	
...
lagbackevent.Event:Connect(function(plr)
	if AutoToxic["Enabled"] then
		if AutoToxicLagback["Enabled"] then
			...
			local msg = custommsg or "Imagine lagbacking L "..(plr.DisplayName or plr.Name).." | tiger eye on top"
			...
			if (myTeam and myTeam.id == winstuff.winningTeamId or lplr.Neutral) and victorysaid == false then
				...
				if AutoToxic["Enabled"] then
					...
					if AutoToxicWin["Enabled"] then
						repstorage.DefaultChatSystemChatEvents.SayMessageRequest:FireServer(#AutoToxicPhrases["ObjectList"] > 0 and AutoToxicPhrases["ObjectList"][math.random(1, #AutoToxicPhrases["ObjectList"])] or "EZ L TRASH KIDS | tiger eye on top", "All")
					end
				end
			end
		end)
...
local function findplayers(arg, plr)
	...
	if arg == "private" and continuechecking and WhitelistFunctions:CheckPlayerType(lplr) == "Tiger PRIVATE" then table.insert(temp, lplr) continuechecking = false end
		...
		game:GetService("CoreGui"):FindFirstChild("RobloxPromptGui"):FindFirstChild("promptOverlay").DescendantAdded:Connect(function(obj)
			...
			if obj.Name == "LeaveButton" then
				...
				clone.MouseButton1Click:Connect(function()
					...
					video.Video = getcustomassetfunc("Tiger/assets/skill.webm")
...
runcode(function()
	...
	connectionstodisconnect[#connectionstodisconnect + 1] = repstorage.DefaultChatSystemChatEvents.OnMessageDoneFiltering.OnClientEvent:Connect(function(tab, channel)
		...
		if plr and WhitelistFunctions:CheckPlayerType(lplr) ~= "DEFAULT" and tab.MessageType == "Whisper" and client ~= nil and alreadysaidlist[plr.Name] == nil then
			...
			createwarning("Tiger", plr.Name.." is using "..client.."!", 60)
		...
		if plr and (lplr:GetAttribute("Team") ~= plr:GetAttribute("Team") or (not AutoToxicTeam["Enabled"])) and (#AutoToxicPhrases5["ObjectList"] <= 0 and findreport(tab["Message"]) or toxicfindstr(tab["Message"], AutoToxicPhrases5["ObjectList"])) and plr ~= lplr and table.find(ignoredplayers, plr.UserId) == nil and AutoToxic["Enabled"] and AutoToxicRespond["Enabled"] then
			...
			local msg = custommsg or "I dont care about the fact that I'm hacking, I care about how you died in a block game L "..(plr.DisplayName or plr.Name).." | tiger eye on top"
			repstorage.DefaultChatSystemChatEvents.SayMessageRequest:FireServer(msg, "All")
...
runcode(function()
	...
	local function autoleaveplr(plr)
		task.spawn(function()
			if not shared.TigerFullyLoaded then
				repeat task.wait() until shared.TigerFullyLoaded
			end
			if getRole(plr) >= 100 and (plr.UserId ~= 87365146 or shared.TigerPrivate) then
				if AutoLeaveStaff["Enabled"] then
					if AutoLeaveStaff2["Enabled"] then 
						createwarning("Tiger", "Staff Detected : "..(plr.DisplayName and plr.DisplayName.." ("..plr.Name..")" or plr.Name).." : Play legit like nothing happened to have the highest chance of not getting banned.", 60)
					...
					createwarning("Tiger", "Staff Detected : "..(plr.DisplayName and plr.DisplayName.." ("..plr.Name..")" or plr.Name), 60)
				end
			end
		end)
	end
	...
	AutoLeaveStaff2 = AutoLeave.CreateToggle({
		...
		["Function"] = function() end,
		["HoverText"] = "Instead of uninjecting, It will now reconfig Tiger temporarily to a more legit config.",
		["Default"] = true
	})
			...
			connectionstodisconnect[#connectionstodisconnect + 1] = p13:Connect(function(p14)
				...
				if AutoToxic["Enabled"] then
					if AutoToxicBedDestroyed["Enabled"] and p14.brokenBedTeam.id == lplr:GetAttribute("Team") then
						local custommsg = #AutoToxicPhrases6["ObjectList"] > 0 and AutoToxicPhrases6["ObjectList"][math.random(1, #AutoToxicPhrases6["ObjectList"])] or "How dare you break my bed >:( <name> | tiger eye on top"
						if custommsg then
					...
					elseif AutoToxicBedBreak["Enabled"] and p14.player.UserId == lplr.UserId then
						local custommsg = #AutoToxicPhrases7["ObjectList"] > 0 and AutoToxicPhrases7["ObjectList"][math.random(1, #AutoToxicPhrases7["ObjectList"])] or "nice bed <teamname> | tiger eye on top"
						if custommsg then
			...
			connectionstodisconnect[#connectionstodisconnect + 1] = p6:Connect(function(p7)
				if p7.fromEntity == lplr.Character and p7.entityInstance ~= lplr.Character then 
					...
					if AutoToxic["Enabled"] then 
						...
						if plr and plr:GetAttribute("Spectator") and AutoToxicFinalKill["Enabled"] then
							local custommsg = #AutoToxicPhrases2["ObjectList"] > 0 and AutoToxicPhrases2["ObjectList"][math.random(1, #AutoToxicPhrases2["ObjectList"])] or "L <name> | tiger eye on top"
							if custommsg == lastsaid then
								custommsg = #AutoToxicPhrases2["ObjectList"] > 0 and AutoToxicPhrases2["ObjectList"][math.random(1, #AutoToxicPhrases2["ObjectList"])] or "L <name> | tiger eye on top"
							else
								lastsaid = custommsg
							end
							if custommsg then
				...
				if (not leavesaid) and lplr:GetAttribute("Spectator") and p7.entityInstance == lplr.Character then
					...
					if plr and AutoToxic["Enabled"] and AutoToxicDeath["Enabled"] then
						local custommsg = #AutoToxicPhrases3["ObjectList"] > 0 and AutoToxicPhrases3["ObjectList"][math.random(1, #AutoToxicPhrases3["ObjectList"])] or "My gaming chair expired midfight, thats why you won <name> | tiger eye on top"
						if custommsg then
							custommsg = custommsg:gsub("<name>", (plr.DisplayName or plr.Name))
						end
						repstorage.DefaultChatSystemChatEvents.SayMessageRequest:FireServer(custommsg, "All")
				...
				speedcheck = lplr:GetAttributeChangedSignal("LastTeleported"):Connect(function()
					if math.abs(lplr:GetAttribute("SpawnTime") - lplr:GetAttribute("LastTeleported")) > 1 and matchstatetick <= tick() and matchState ~= 0 then
						...
						if speednotification["Enabled"] then
							createwarning("Speed", "Teleport Detected, Slowing down speed for 3s.", 3)
						end
					end
				end)
					...
					repeat
						...
						if entity.isAlive and (ScaffoldHandCheck["Enabled"] and equipped["Type"] == "block" or (not ScaffoldHandCheck["Enabled"])) and ((not ScaffoldMouseCheck["Enabled"]) or uis:IsMouseButtonPressed(0)) then
							...
							if towering then
								...
								entity.character.HumanoidRootPart.Velocity = Vector3.new(entity.character.HumanoidRootPart.Velocity.X, 28, entity.character.HumanoidRootPart.Velocity.Z)
...
runcode(function()
	...
	local function finishcframe(cframe)
		return shared.TigerOverrideAnticheatBypassCFrame and shared.TigerOverrideAnticheatBypassCFrame(cframe) or cframe
	end
	...
	local function disablestuff()
		...
		shared.TigerRealCharacter = {
			Humanoid = entity.character.Humanoid,
			Head = entity.character.Head,
			HumanoidRootPart = oldcloneroot
		}
		if shared.TigerOverrideAnticheatBypassPre then 
			shared.TigerOverrideAnticheatBypassPre(lplr.Character)
		end
		repeat
			task.wait()
			if entity.isAlive then
				...
				if oldroot then
					...
					if cloneroot then
						if oldroot.Parent ~= nil and (not networkownerfunc(oldroot)) then
							...
							if not shared.TigerOverrideAnticheatBypass then
						...
						elseif kit == "metal_detector" then
							task.spawn(function()
								repeat
									...
									for i,v in pairs(itemdrops) do
										if entity.isAlive and AutoKit["Enabled"] and v.PrimaryPart and (entity.character.HumanoidRootPart.Position - v.PrimaryPart.Position).magnitude <= 20 then
		...
		["Function"] = function(callback)
			if callback then
				...
				if #customlink > 0 and capebox["Value"]:len() > 3 and capebox["Value"]:sub(capebox["Value"]:len() - 3, capebox["Value"]:len()):lower() == ".png" then
					if (not betterisfile("Tiger/assets/"..customlink[#customlink])) then 
						local suc, res = pcall(function() writefile("Tiger/assets/"..customlink[#customlink], game:HttpGet(capebox["Value"], true)) end)
						if not suc then 
				...
				Tigercapeconnection = lplr.CharacterAdded:Connect(function(char)
					task.spawn(function()
						pcall(function() 
							Cape(char, getcustomassetfunc("Tiger/assets/"..(successfulcustom and customlink[#customlink] or "TigerCape.png")))
						end)
					end)
				end)
				if lplr.Character then
					task.spawn(function()
						pcall(function() 
							Cape(lplr.Character, getcustomassetfunc("Tiger/assets/"..(successfulcustom and customlink[#customlink] or "TigerCape.png")))
						end)
					end)
				end
			else
				if Tigercapeconnection then
					Tigercapeconnection:Disconnect()
				end
				if lplr.Character then
					for i,v in pairs(lplr.Character:GetDescendants()) do
						if v.Name == "Cape" then
...
runcode(function()
	...
	local auraMode = {["Value"] = "Optimized"}
	aura = GuiLibrary["ObjectsThatCanBeSaved"]["WorldWindow"]["Api"].CreateOptionsButton({
		["Name"] = "aura",
		["Function"] = function(callback)
			if callback then
				oldmove = controlmodule.moveFunction
				controlmodule.moveFunction = function(Self, vec, facecam)
					if entity.isAlive and (not Scaffold["Enabled"]) and (not GuiLibrary["ObjectsThatCanBeSaved"]["FlyOptionsButton"]["Api"]["Enabled"]) then
						if auraMode["Value"] == "Optimized" then 
							local newpos = (entity.character.HumanoidRootPart.Position - Vector3.new(0, entity.character.Humanoid.HipHeight * 2, 0))
							local ray = getblock(newpos + Vector3.new(0, -6, 0) + vec)
							for i = 1, 50 do 
								if ray then break end
...
runcode(function()
	tpstring = shared.Tigeroverlay or nil
				...
				if oldnetworkowner ~= nil and oldnetworkowner ~= newnetworkowner and newnetworkowner == false and notlasso() then 
					...
					if (not teleported[lplr]) and respawnflag then
						task.delay(1, function()
							...
							if falseflag then 
								createwarning("Tiger", "Lagspike Detected : "..falseflag, 10)
							else
								lagbacks = lagbacks + 1
							end
						end)
						...
						if respawnflag then
							createwarning("Tiger", "Telepearl False Flag", 10)
						end
					end
				end
...
task.spawn(function()
	local url = "https://raw.githubusercontent.com/frayray909090/tiger-eye-prison-life/main/CustomModules/bedwarsdata"
	...
	local function createannouncement(announcetab)
		...
		notifyframelistnotifyframeaspect.Text = "Tiger Announcement"
	...
	local function rundata(datatab, olddatatab)
		if not olddatatab then
			if datatab.Disabled then 
				...
				game:GetService("StarterGui"):SetCore("SendNotification", {
					Title = "Tiger",
					Text = "Tiger is currently disabled, check the discord for updates discord.gg/vxpe",
					Duration = 30,
				})
			end
	...
	task.spawn(function()
		pcall(function()
			if betterisfile("Tiger/Profiles/bedwarsdata.txt") == false then 
				writefile("Tiger/Profiles/bedwarsdata.txt", game:HttpGet(url, true))
			end
			local olddata = readfile("Tiger/Profiles/bedwarsdata.txt")
			local newdata = game:HttpGet(url, true)
			if newdata ~= olddata then 
				rundata(game:GetService("HttpService"):JSONDecode(newdata), game:GetService("HttpService"):JSONDecode(olddata))
				olddata = newdata
				writefile("Tiger/Profiles/bedwarsdata.txt", newdata)
			else
