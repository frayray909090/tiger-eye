...
local function getNametagString(plr)
	...
	if WhitelistFunctions:CheckPlayerType(plr) == "Tiger PRIVATE" then
		nametag = '<font color="rgb(127, 0, 255)">[Tiger PRIVATE] '..(plr.DisplayName or plr.Name)..'</font>'
	end
		...
		spawn(function()
			if plr.Character and plr.Character ~= oldchar then
				spawn(function()
					pcall(function() 
						bedwars["getEntityTable"]:getEntity(plr):setNametag(nametag)
						Cape(plr.Character, getcustomassetfunc("Tiger/assets/TigerCape.png"))
					end)
				end)
			end
		end)
...
teleportfunc = lplr.OnTeleport:Connect(function(State)
    if (not teleported) then
		teleported = true
		if shared.Tigeroverlay then
			queueteleport('shared.Tigeroverlay = "'..shared.Tigeroverlay..'"')
		end
    end
end)
		...
		if callback then
			spawn(function()
				repeat
					...
					firstqueue = false
					if shared.Tigerteammembers and bedwars["ClientStoreHandler"]:getState().Party then
						repeat task.wait() until #bedwars["ClientStoreHandler"]:getState().Party.members >= shared.Tigerteammembers or JoinQueue["Enabled"] == false
					end
...
runcode(function()
	local tpstring = shared.Tigeroverlay or nil
