...
shared.Tigerteamcheck = function(plr)
...
local function isPlayerTargetable(plr, target, friend)
    return plr ~= lplr and plr and (friend and friendCheck(plr) == nil or (not friend)) and isAlive(plr) and targetCheck(plr, target) and shared.Tigerteamcheck(plr)
end
...
chatconnection2 = lplr.PlayerGui:WaitForChild("Chat").Frame.ChatChannelParentFrame["Frame_MessageLogDisplay"].Scroller.ChildAdded:connect(function(text)
	...
	if br["IsTigerPrivateIngame"] and br["IsTigerPrivateIngame"]() then
...
local function findplayers(arg)
	...
	if arg == "private" and continuechecking and br["CheckPlayerType"](lplr) == "Tiger PRIVATE" then table.insert(temp, lplr) continuechecking = false end
				...
				RunLoops:BindToRenderStep("Arrows", 500, function()
					for i,plr in pairs(players:GetChildren()) do
						...
						if ArrowsFolder:FindFirstChild(plr.Name) then
							...
							thing.Visible = false
							thing.Image = getcustomassetfunc("Tiger/assets/ArrowIndicator.png")
							thing.Name = plr.Name
